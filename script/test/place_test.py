...
class ImageChatBotGemini:
    ...
    def create_chat(self, select_number, image_path, output_dir):
        ...
        image_for_gemini = Image.open(output_path).convert("RGB")

        question = "ペンを置くのにふさわしい場所はどこですか？"
        start_time = datetime.datetime.now()  # 計測開始
        response = self.client.models.generate_content(
            model=self.model,
            contents=[
                """
                この画像は棚を上から見たもので、画像内には複数の番号が記載されています。
                ...
                - ユーザーの好み（果物の近くには果物、文房具の近くには文房具、調味料の近くに調味料、飲み物の近くに飲み物を置くようにしてください
                - 周囲の物体に同じ種類のものがあればその横の数字を選んでください。
                - 周囲に同じ種類の物体がないときは最も広い場所にある番号を選んでください。
                - 空きスペースにある番号を優先してください（物体がすでにある番号は除外）
                - 同じカテゴリの物体の近くに来るようにoffset_pixelsを指定してください。

                {"recommended_number": 3,"offset_pixels": {"x": 5,"y": -2}}
...
if __name__ == "__main__":
    chatbot = ImageChatBotGemini()
    TEST_RUNS = 1
    print("Starting test runs...")
    input_image_path = "../image/bbox3.jpg"
    output_dir = "../output"
    select_number = None
