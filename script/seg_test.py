...
TARGET_DIR = r'../output/segumentation_results/'
...
def overlay_mask_on_img(img: Image.Image, mask: np.ndarray, color: str, alpha: float = 0.7) -> Image.Image:
...
def main_segmentation():
    ...
    # Save result
    idx = get_next_segmentation_image_index()
    out_path = os.path.join(TARGET_DIR, f'segmentation_result_{idx}.jpg')
